...
#endif
#ifdef ENABLE_LWM2M
extern int luaopen_lwm2m(lua_State* L);
#endif
...
/* 可选平台模块列表 */
static const luaL_Reg optional_platform_modules[] = {
#ifdef ENABLE_CAMERA
...
#ifdef ENABLE_LCD
    {"lcd",   luaopen_lcd},          /* LCD显示模块 */
#endif

#ifdef ENABLE_LWM2M
    {"lwm2m", luaopen_lwm2m},         /* LwM2M协议 */
#endif
...
void platform_modules_register(lua_State* L)
{
    ...
    /* 注册可选平台模块 */
    for (lib = optional_platform_modules; lib->func; lib++) {
        register_module(L, lib->name, lib->func);
        count++;
    }
}
