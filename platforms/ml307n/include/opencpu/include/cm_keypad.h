...
typedef enum
{
    CM_KEYPAD_CFGCMD_COL,    /*!< 配置按键矩阵的列号, 类型: uint8_t数组, 例如：unit8_t cm_col[3]={1,2,3} */
    CM_KEYPAD_CFGCMD_ROL,    /*!< 配置按键矩阵的行号, 类型: uint8_t数组, 例如：unit8_t cm_rol[3]={1,2,3} */
    CM_KEYPAD_CFGCMD_COLLEN, /*!< 配置按键矩阵的列长度, 类型: uint8_t, 取值范围: 1-32 */
    CM_KEYPAD_CFGCMD_ROLLEN, /*!< 配置按键矩阵的行长度, 类型: uint8_t, 取值范围: 1-32 */
    CM_KEYPAD_CFGCMDMAX,      /*!< 配置命令最大值，用作边界标记 */
}cm_keypad_cfgcmd_e;
...
/** 配置 */
typedef struct{
    cm_keypad_kp_mkin_e cm_col[KPD_ROW_NUM];
    cm_keypad_kp_mkout_e cm_row[KPD_COL_NUM];
 ...
 * @brief 配置按键矩阵参数
 *
 * @param [in] cmd   配置命令，参考cm_keypad_cfgcmd_e枚举定义
 * @param [in] value 配置值指针，根据cmd不同，指向不同类型的值
 *
 * @return
 *   =  0 - 成功
 *   < 0 - 失败, 返回值为错误码
 *
 * @details 该函数用于配置按键矩阵参数，需在cm_keypad_init函数之前使用
 */
int32_t cm_keypad_set_config(cm_keypad_cfgcmd_e cmd, void *value);

/**
 * @brief 获取按键矩阵配置参数
 *
 * @param [in]  cmd   配置命令，参考cm_keypad_cfgcmd_e枚举定义
 * @param [out] value 配置值指针，根据cmd不同，指向不同类型的值
 *
 * @return
 *   =  0 - 成功
 *   = -1 - 失败
 *
 * @details 用于获取当前按键矩阵的配置参数
 */
int32_t cm_keypad_get_config(cm_keypad_cfgcmd_e cmd, void *value);
