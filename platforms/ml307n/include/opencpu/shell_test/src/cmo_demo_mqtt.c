...
static int __mqtt_manager_default_publish_cb(cm_mqtt_client_t* client, unsigned short msgid, char* topic, int total_len, int payload_len, char* payload)
{
    ...
    /* 由于测试示例限制打印长度 */
    int printf_size = 0;
    while(printf_size < payload_len)
    {
        osPrintf("\r\n[MQTT]CM MQTT index[%d] , recv: %.*s\r\n", i, 
            payload_len - printf_size < 220 ? payload_len - printf_size : 220, payload + printf_size);
        printf_size += 220;
    }
...
void cm_test_mqtt(char len, char** cmd) 
{
    ...
    switch(opt)
    {
        ...
        case 4:
        {
            ...
            char *pubmessage = cmd[3];
            int ret = cm_mqtt_client_publish(_mqtt_client[0], cmd[2], pubmessage, strlen(pubmessage), publish_flags);
...
NR_SHELL_CMD_EXPORT(cm_test_mqtt_sample, cm_test_mqtt);
#endif
