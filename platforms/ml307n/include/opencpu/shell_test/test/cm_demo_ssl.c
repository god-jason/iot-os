...
#define PRINT_ONE_LINE_LENGTH (83)
/****************************************************************************
 * Private Types
 ****************************************************************************/
 ...
 * Public Functions
 ****************************************************************************/
typedef struct ssl_demo_param
{
    char len;
    char** cmd;
} ssl_demo_param_t;

void cmo_ssl_demo(void *param)
{
    ssl_demo_param_t *demo_param = (ssl_demo_param_t *)param;
    cmo_test_ssl(demo_param->len, demo_param->cmd);
}
void shell_cmo_ssl(char len, char** cmd)
{
    ssl_demo_param_t param = {0};
    param.len = len;
    param.cmd = cmd;
    ...
    osThreadNew(cmo_ssl_demo, (void *)&param, &attr);
    osDelay(200);
}

/* 执行测试命令 */
void cmo_test_ssl(char len, char** cmd_ssl)
{
    /* ssl [cmd]... */
    int i = 0;
    if(len < 2)
    ...
    const char *cmd = (const char *)cmd_ssl[1];
    if (cmd == NULL)
    {
        cm_demo_printf("invalid param\n");
        return;
    ...
    else if (strncmp(cmd, STR_ITEM("cipher")) == 0)
    {
        ...
        if (data)
        {
            ...
            while (remain_size)
            {
                ...
                if (remain_size > PRINT_ONE_LINE_LENGTH)
                {
                
                    for (int i = 0; i < PRINT_ONE_LINE_LENGTH; i++)
                    {
                        len += snprintf((char *)data + len, (size * 7 + 1 - len), "0x%x,", list[list_offset + i]);
                    }
                    
                    list_offset += PRINT_ONE_LINE_LENGTH;
                    remain_size -= PRINT_ONE_LINE_LENGTH;
                    osPrintf("%s", data);
                }
                else
                {
...
NR_SHELL_CMD_EXPORT(ssl, shell_cmo_ssl);
#endif
