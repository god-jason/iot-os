...
uint32_t amr_file_read_pcm_frames_s16(void *amrHandle, uint32_t framesToRead, uint8_t* pBufferOut);
int amr_release(void);
void decoder_release(void* state);
int cm_auido_amr_file_decoder_init(void **decoder, VFS_File *fd, uint8_t *dataBits, uint32_t *sampleRate, uint8_t *chnls);
