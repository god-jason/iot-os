...
{
    if(sampleRate == NULL || dataBits == NULL || frameSize == NULL)
    {
        return;
    }
...
{
    if(g_sampleRate != 0 && sampleRate != g_sampleRate)
    {
        CD_LOG("codec has inited a different sampleRate");
    }
    else
    {
        g_sampleRate = sampleRate;
    }
    if(g_dataBits != 0 && dataBits != g_dataBits)
    {
        CD_LOG("codec has inited a different dataBits");
    }
    else
    {
        g_dataBits = dataBits;
    }
    if(g_frameSize != 0 && frameSize != g_frameSize)
    {
        CD_LOG("codec has inited a different frameSize");
    }
    else
    {
