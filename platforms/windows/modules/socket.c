...
static void socket_wsa_init(void) {
...
static socket_ctx_t* socket_find_ctx_by_sock(SOCKET sock) {
...
static DWORD WINAPI socket_poll_thread(LPVOID arg) {
    ...
    while (g_poll_thread_running) {
        ...
        int ret = select((int)max_fd + 1, &read_set, NULL, &error_set, &timeout);
        if (ret < 0) {
            iot_task_delay(10);
            continue;
        }
...
static int iot_socket_write(lua_State* L) {
    ...
    int ret = send(ctx->sock, data, (int)len, 0);
    if (ret == SOCKET_ERROR) {
        lua_pushnil(L);
        lua_pushinteger(L, WSAGetLastError());
        return 2;
...
static int iot_socket_setsockopt(lua_State* L) {
    ...
    int ret = setsockopt(ctx->sock, level, optname, (const char*)&optval, sizeof(optval));
    if (ret == SOCKET_ERROR) {
        lua_pushboolean(L, 0);
        lua_pushinteger(L, WSAGetLastError());
        return 2;
