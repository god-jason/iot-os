...
static int luaopen_uart_setup(lua_State* L) {
    ...
    LOG_DEBUG("setup: id=%d, baudrate=%d", id, baudrate);
    
    if (id < 0 || id >= IOT_UART_MAX) {
        LOG_ERROR("setup failed: invalid id=%d", id);
        lua_pushboolean(L, 0);
        return 1;
    }
...
static int luaopen_uart_close(lua_State* L) {
    ...
    LOG_DEBUG("close: id=%d", id);
    
    if (id < 0 || id >= IOT_UART_MAX) {
        LOG_ERROR("close failed: invalid id=%d", id);
        lua_pushboolean(L, 0);
        return 1;
    }
    
    if (!g_uart_ctx[id].inited) {
        LOG_WARN("close: port not initialized id=%d", id);
        lua_pushboolean(L, 1);
        return 1;
    }
...
static int luaopen_uart_write(lua_State* L) {
    ...
    LOG_TRACE("write: id=%d, len=%zu", id, len);
    
    if (id < 0 || id >= IOT_UART_MAX) {
        LOG_ERROR("write failed: invalid id=%d", id);
        lua_pushnil(L);
        return 1;
    }
    
    if (!g_uart_ctx[id].inited) {
        LOG_ERROR("write failed: port not initialized id=%d", id);
        lua_pushnil(L);
        return 1;
    }
    ...
    if (!WriteFile(g_uart_ctx[id].hCom, data, (DWORD)len, &written, NULL)) {
        LOG_ERROR("write failed: WriteFile error id=%d", id);
        lua_pushnil(L);
    } else {
        LOG_DEBUG("write success: id=%d, written=%lu", id, written);
...
static int luaopen_uart_read(lua_State* L) {
    ...
    LOG_TRACE("read: id=%d, len=%d", id, len);
    
    if (id < 0 || id >= IOT_UART_MAX) {
        LOG_ERROR("read failed: invalid id=%d", id);
        lua_pushnil(L);
        return 1;
    }
    
    if (!g_uart_ctx[id].inited) {
        LOG_ERROR("read failed: port not initialized id=%d", id);
        lua_pushnil(L);
        return 1;
    }
    
    if (len <= 0) {
        LOG_WARN("read: invalid len=%d", len);
        lua_pushnil(L);
        return 1;
    }
    ...
    if (!buff) {
        LOG_ERROR("read failed: malloc error len=%d", len);
        lua_pushnil(L);
        return 1;
    }
    ...
    if (!ReadFile(g_uart_ctx[id].hCom, buff, (DWORD)len, &read, NULL)) {
        LOG_ERROR("read failed: ReadFile error id=%d", id);
        iot_free(buff);
        lua_pushnil(L);
        return 1;
    }
    ...
    if (read == 0) {
        LOG_DEBUG("read: no data id=%d", id);
        iot_free(buff);
        lua_pushnil(L);
        return 1;
    }
    
    LOG_DEBUG("read success: id=%d, read=%lu", id, read);
