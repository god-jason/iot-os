...
int iot_rtc_get_datetime(iot_rtc_time_t *dt)
{
    ...
    ret = cm_rtc_get_datetime(&rtc_time);
    if (ret != 0) {
        return ret;
    }
...
int iot_rtc_set_datetime(const iot_rtc_time_t *dt)
{
    ...
    return cm_rtc_set_datetime(&rtc_time);
}
...
{
    return cm_rtc_set_timezone(zone);
}
...
{
    return cm_rtc_register_alarm(sec, arg, (cm_rtc_alarm_cb_t)cb);
}
...
{
    return cm_rtc_unregister_alarm(id);
}