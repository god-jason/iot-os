...
static iot_modbus_err_t iot_modbus_rtu_finalize(uint8_t *out, size_t out_cap, size_t data_len, size_t *out_len)
{
    uint16_t crc;

    if (!out || !out_len || data_len + 2 > out_cap || data_len + 2 > IOT_MODBUS_RTU_MAX_FRAME) {
        return IOT_MODBUS_ERR_BUF;
    }

    crc = iot_modbus_crc16(out, data_len);
    out[data_len] = (uint8_t)(crc & 0xFF);
    out[data_len + 1] = (uint8_t)((crc >> 8) & 0xFF);
    *out_len = data_len + 2;
    return IOT_MODBUS_OK;
}
...
bool iot_modbus_crc16_verify(const uint8_t *frame, size_t len)
{
    ...
    if (!frame || len < IOT_MODBUS_RTU_MIN_FRAME) {
        return false;
    }

    crc = iot_modbus_crc16(frame, len - 2);
                                   ...
                                   uint8_t *out, size_t out_cap, size_t *out_len)
{
    if (!out || !out_len || addr == 0 || qty == 0) {
        return IOT_MODBUS_ERR_PARAM;
    }

    if (fc != IOT_MODBUS_FC_READ_COILS &&
        fc != IOT_MODBUS_FC_READ_DISCRETE_INPUTS &&
        fc != IOT_MODBUS_FC_READ_HOLDING_REGISTERS &&
        fc != IOT_MODBUS_FC_READ_INPUT_REGISTERS) {
        return IOT_MODBUS_ERR_PARAM;
    }
    ...
    return iot_modbus_rtu_finalize(out, out_cap, 6, out_len);
}
                                             ...
                                             uint8_t *out, size_t out_cap, size_t *out_len)
{
    ...
    out[0] = addr;
    out[1] = IOT_MODBUS_FC_WRITE_SINGLE_REGISTER;
                                              ...
                                              uint8_t *out, size_t out_cap, size_t *out_len)
{
    ...
    byte_count = count * 2;
    if (7 + byte_count + 2 > out_cap || 7 + byte_count + 2 > IOT_MODBUS_RTU_MAX_FRAME) {
        return IOT_MODBUS_ERR_BUF;
    }

    out[0] = addr;
    out[1] = IOT_MODBUS_FC_WRITE_MULTIPLE_REGISTERS;
    out[2] = (uint8_t)((start >> 8) & 0xFF);
    out[3] = (uint8_t)(start & 0xFF);
    ...
    return iot_modbus_rtu_finalize(out, out_cap, pos, out_len);
}
                                         ...
                                         uint8_t *out, size_t out_cap, size_t *out_len)
{
    ...
    out[0] = addr;
    out[1] = IOT_MODBUS_FC_WRITE_SINGLE_COIL;
                                          ...
                                          uint8_t *out, size_t out_cap, size_t *out_len)
{
    ...
    out[0] = addr;
    out[1] = IOT_MODBUS_FC_WRITE_MULTIPLE_COILS;
    out[2] = (uint8_t)((start >> 8) & 0xFF);
    out[3] = (uint8_t)(start & 0xFF);
...
{
    if (!frame || !out || len < IOT_MODBUS_RTU_MIN_FRAME) {
        return IOT_MODBUS_ERR_PARAM;
    }

    if (!iot_modbus_crc16_verify(frame, len)) {
        return IOT_MODBUS_ERR_CRC;
    }
    ...
    if (out->fc & 0x80) {
        if (len < 5) {
            return IOT_MODBUS_ERR_FORMAT;
        }
        out->is_exception = true;
        out->exception_code = frame[2];
        out->payload = frame + 2;
        out->payload_len = 1;
        return IOT_MODBUS_ERR_EXCEPTION;
    }
...
static uint16_t iot_modbus_swap16(uint16_t v)
{
...
iot_modbus_err_t iot_modbus_regs_to_uint32(uint16_t hi, uint16_t lo, const char *order, uint32_t *out)
{
    ...
    if (order == NULL || order[0] == '\0' || strcmp(order, "ABCD") == 0) {
        ...
        value = ((uint32_t)iot_modbus_swap16(w0) << 16) | iot_modbus_swap16(w1);
    } else if (strcmp(order, "DCBA") == 0) {
        value = ((uint32_t)iot_modbus_swap16(w1) << 16) | iot_modbus_swap16(w0);
    } else {
        return IOT_MODBUS_ERR_PARAM;
    }
...
{
    uint32_t u;
    iot_modbus_err_t err = iot_modbus_regs_to_uint32(hi, lo, order, &u);
...
iot_modbus_err_t iot_modbus_regs_to_float32(uint16_t hi, uint16_t lo, const char *order, float *out)
{
    ...
    if (err != IOT_MODBUS_OK || !out) {
        return err != IOT_MODBUS_OK ? err : IOT_MODBUS_ERR_PARAM;
    }
