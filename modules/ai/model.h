...
/* 推理后端类型 */
typedef enum {
    IOT_MODEL_BACKEND_AUTO   = 0,  /* 自动选择最优后端 */
    IOT_MODEL_BACKEND_TFLITE = 1,  /* TFLite (ESP32 / Linux) */
    IOT_MODEL_BACKEND_ONNX   = 2,  /* ONNX Runtime (Linux / Windows) */
    IOT_MODEL_BACKEND_NONE   = 3   /* 无可用的推理后端 */
} iot_model_backend_t;

/* 模型精度类型 */
typedef enum {
    IOT_MODEL_PRECISION_AUTO = 0,  /* 自动检测 */
    IOT_MODEL_PRECISION_FP32 = 1,  /* 32 位浮点 */
    IOT_MODEL_PRECISION_FP16 = 2,  /* 16 位浮点 */
    IOT_MODEL_PRECISION_INT8 = 3,  /* 8 位整数量化 */
    IOT_MODEL_PRECISION_INT16= 4   /* 16 位整数量化 */
} iot_model_precision_t;

/* Tensor 数据类型 */
typedef enum {
    IOT_MODEL_DTYPE_FLOAT32 = 0,
    IOT_MODEL_DTYPE_FLOAT16 = 1,
    IOT_MODEL_DTYPE_INT32   = 2,
    IOT_MODEL_DTYPE_INT8    = 3,
    IOT_MODEL_DTYPE_UINT8   = 4,
    IOT_MODEL_DTYPE_INT16   = 5,
    IOT_MODEL_DTYPE_BOOL    = 6,
    IOT_MODEL_DTYPE_STRING  = 7
} iot_model_dtype_t;

/* Tensor 信息 */
typedef struct {
    char     name[64];          /* Tensor 名称 */
    iot_model_dtype_t dtype;        /* 数据类型 */
    int      dims[8];           /* 维度数组 */
    int      ndim;              /* 维度数 */
    size_t   size_bytes;        /* 数据大小 (字节) */
    void*    data;              /* 数据指针 (模型内部管理) */
} iot_model_tensor_info_t;

/* 模型句柄 (不透明指针) */
typedef struct iot_model_ctx iot_model_t;
 ...
 * @param backend 指定后端 (IOT_MODEL_BACKEND_AUTO 自动选择)
 * @param precision 期望精度 (IOT_MODEL_PRECISION_AUTO 自动检测)
