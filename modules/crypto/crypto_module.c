 ...
 */
static int luaopen_iot_crypto_hash(lua_State* L) {
    ...
    iot_crypto_hash_type_t type;
    if (strcmp(algo, "md5") == 0) {
        type = IOT_CRYPTO_HASH_MD5;
    } else if (strcmp(algo, "sha1") == 0 || strcmp(algo, "sha") == 0) {
        type = IOT_CRYPTO_HASH_SHA1;
    } else if (strcmp(algo, "sha256") == 0) {
        type = IOT_CRYPTO_HASH_SHA256;
    } else if (strcmp(algo, "sha224") == 0) {
        type = IOT_CRYPTO_HASH_SHA224;
    } else if (strcmp(algo, "sha384") == 0) {
        type = IOT_CRYPTO_HASH_SHA384;
    } else if (strcmp(algo, "sha512") == 0) {
        type = IOT_CRYPTO_HASH_SHA512;
    } else if (strcmp(algo, "sm3") == 0) {
        type = IOT_CRYPTO_HASH_SM3;
    } else {
    ...
    int ret = iot_crypto_hash(type, (const uint8_t*)data, inputlen, digest, &digestlen);
    if (input) iot_free(input);
 ...
 */
static int luaopen_iot_crypto_hmac(lua_State* L) {
    const char* algo = luaL_checkstring(L, 1);
    size_t keylen;
    ...
    int ret = iot_crypto_hmac(type, (const uint8_t*)key, keybinlen,
 ...
 */
static int luaopen_iot_crypto_encrypt(lua_State* L) {
    const char* algo = luaL_checkstring(L, 1);
    size_t keylen;
    const char* key = luaL_checklstring(L, 2, &keylen);
    ...
    if (strcmp(algo, "aes128-ecb") == 0) type = IOT_CRYPTO_CIPHER_AES_128_ECB;
    else if (strcmp(algo, "aes128-cbc") == 0) type = IOT_CRYPTO_CIPHER_AES_128_CBC;
    else if (strcmp(algo, "aes128-ctr") == 0) type = IOT_CRYPTO_CIPHER_AES_128_CTR;
    else if (strcmp(algo, "aes128-gcm") == 0) type = IOT_CRYPTO_CIPHER_AES_128_GCM;
    else if (strcmp(algo, "aes192-ecb") == 0) type = IOT_CRYPTO_CIPHER_AES_192_ECB;
    else if (strcmp(algo, "aes192-cbc") == 0) type = IOT_CRYPTO_CIPHER_AES_192_CBC;
    else if (strcmp(algo, "aes192-ctr") == 0) type = IOT_CRYPTO_CIPHER_AES_192_CTR;
    else if (strcmp(algo, "aes192-gcm") == 0) type = IOT_CRYPTO_CIPHER_AES_192_GCM;
    else if (strcmp(algo, "aes256-ecb") == 0) type = IOT_CRYPTO_CIPHER_AES_256_ECB;
    else if (strcmp(algo, "aes256-cbc") == 0) type = IOT_CRYPTO_CIPHER_AES_256_CBC;
    else if (strcmp(algo, "aes256-ctr") == 0) type = IOT_CRYPTO_CIPHER_AES_256_CTR;
    else if (strcmp(algo, "aes256-gcm") == 0) type = IOT_CRYPTO_CIPHER_AES_256_GCM;
    else if (strcmp(algo, "sm4-ecb") == 0) type = IOT_CRYPTO_CIPHER_SM4_ECB;
    else if (strcmp(algo, "sm4-cbc") == 0) type = IOT_CRYPTO_CIPHER_SM4_CBC;
    else if (strcmp(algo, "sm4-ctr") == 0) type = IOT_CRYPTO_CIPHER_SM4_CTR;
    else if (strcmp(algo, "sm4-gcm") == 0) type = IOT_CRYPTO_CIPHER_SM4_GCM;
    else {
        lua_pushnil(L);
    ...
    int ret = iot_crypto_encrypt(type, (const uint8_t*)key, keybinlen,
 ...
 */
static int luaopen_iot_crypto_decrypt(lua_State* L) {
    const char* algo = luaL_checkstring(L, 1);
    size_t keylen;
    const char* key = luaL_checklstring(L, 2, &keylen);
    const char* iv = lua_isnoneornil(L, 3) ? NULL : luaL_checklstring(L, 3, NULL);
    ...
    size_t outlen_real = outlen;
    int ret = iot_crypto_decrypt(type, (const uint8_t*)key, keybinlen,
                             (const uint8_t*)iv, ivbinlen,
                             NULL, 0,
                             (const uint8_t*)data, databinlen,
                             out, &outlen_real);
 ...
 */
static int luaopen_iot_crypto_rand(lua_State* L) {
    ...
    if (iot_crypto_rand_bytes(buf, len) != 0) {
        iot_free(buf);
        lua_pushnil(L);
 ...
 */
static int luaopen_iot_crypto_hex2bin(lua_State* L) {
    size_t hexlen;
 ...
 * @return 十六进制字符串
 */
static int luaopen_iot_crypto_bin2hex(lua_State* L) {
    size_t binlen;
 ...
 * @return Base64 编码字符串
 */
static int luaopen_iot_crypto_base64_encode(lua_State* L) {
    size_t datalen;
    const char* data = luaL_checklstring(L, 1, &datalen);

    size_t outlen = iot_crypto_base64_encode_len(datalen);
    ...
    int ret = iot_crypto_base64_encode((const uint8_t*)data, datalen, out, &reallen);
 ...
 * @return 解码后的二进制数据
 */
static int luaopen_iot_crypto_base64_decode(lua_State* L) {
    size_t datalen;
    const char* data = luaL_checklstring(L, 1, &datalen);

    size_t outlen = iot_crypto_base64_decode_len(datalen);
    uint8_t* out = (uint8_t*)iot_malloc(outlen);
    if (!out) {
        lua_pushnil(L);
        lua_pushstring(L, "out of memory");
    ...
    size_t reallen = outlen;
    int ret = iot_crypto_base64_decode(data, datalen, out, &reallen);
...
/* X509 证书 userdata 元表 */
#define IOT_X509_CERT_METATABLE "crypto.x509_cert"

static int luaopen_iot_crypto_x509_cert_gc(lua_State* L) {
    iot_crypto_x509_cert_t** cert = (iot_crypto_x509_cert_t**)luaL_checkudata(L, 1, IOT_X509_CERT_METATABLE);
    if (cert && *cert) {
        iot_crypto_x509_free(*cert);
        *cert = NULL;
    }
    return 0;
}

static int luaopen_iot_crypto_x509_cert_tostring(lua_State* L) {
    iot_crypto_x509_cert_t** cert = (iot_crypto_x509_cert_t**)luaL_checkudata(L, 1, IOT_X509_CERT_METATABLE);
    if (!cert || !*cert) {
 ...
 */
static int luaopen_iot_crypto_x509_parse_pem(lua_State* L) {
    size_t pemlen;
    const char* pem = luaL_checklstring(L, 1, &pemlen);

    iot_crypto_x509_cert_t* cert = iot_crypto_x509_parse_pem(pem, pemlen);
    if (!cert) {
        lua_pushnil(L);
    ...
    iot_crypto_x509_cert_t** udata = (iot_crypto_x509_cert_t**)lua_newuserdata(L, sizeof(iot_crypto_x509_cert_t*));
    *udata = cert;

    if (luaL_newmetatable(L, IOT_X509_CERT_METATABLE)) {
        lua_pushcfunction(L, luaopen_iot_crypto_x509_cert_gc);
        lua_setfield(L, -2, "__gc");
        lua_pushcfunction(L, luaopen_iot_crypto_x509_cert_tostring);
 ...
 * @return 主题字符串
 */
static int luaopen_iot_crypto_x509_cert_get_subject(lua_State* L) {
    iot_crypto_x509_cert_t** cert = (iot_crypto_x509_cert_t**)luaL_checkudata(L, 1, IOT_X509_CERT_METATABLE);
    if (!cert || !*cert) {
        lua_pushnil(L);
        return 1;
    ...
    char* subject = iot_crypto_x509_get_subject(*cert);
    if (subject) {
 ...
 * @return 颁发者字符串
 */
static int luaopen_iot_crypto_x509_cert_get_issuer(lua_State* L) {
    iot_crypto_x509_cert_t** cert = (iot_crypto_x509_cert_t**)luaL_checkudata(L, 1, IOT_X509_CERT_METATABLE);
    if (!cert || !*cert) {
        lua_pushnil(L);
        return 1;
    ...
    char* issuer = iot_crypto_x509_get_issuer(*cert);
    if (issuer) {
 ...
 * @return 序列号字符串
 */
static int luaopen_iot_crypto_x509_cert_get_serial_number(lua_State* L) {
    iot_crypto_x509_cert_t** cert = (iot_crypto_x509_cert_t**)luaL_checkudata(L, 1, IOT_X509_CERT_METATABLE);
    if (!cert || !*cert) {
        lua_pushnil(L);
        return 1;
    ...
    char* serial = iot_crypto_x509_get_serial_number(*cert);
    if (serial) {
 ...
 * @return 通用名称字符串
 */
static int luaopen_iot_crypto_x509_cert_get_common_name(lua_State* L) {
    iot_crypto_x509_cert_t** cert = (iot_crypto_x509_cert_t**)luaL_checkudata(L, 1, IOT_X509_CERT_METATABLE);
    if (!cert || !*cert) {
        lua_pushnil(L);
        return 1;
    ...
    char* cn = iot_crypto_x509_get_common_name(*cert);
    if (cn) {
        lua_pushstring(L, cn);
        free(cn);
    } else {
...
/* X509 证书方法表 */
static const luaL_Reg iot_x509_cert_methods[] = {
    { "get_subject",      luaopen_iot_crypto_x509_cert_get_subject },
    { "get_issuer",       luaopen_iot_crypto_x509_cert_get_issuer },
    { "get_serial_number",luaopen_iot_crypto_x509_cert_get_serial_number },
    { "get_common_name",  luaopen_iot_crypto_x509_cert_get_common_name },
    { NULL,               NULL }
};
 ...
 */
static int luaopen_iot_crypto_pbkdf2_sha256(lua_State* L) {
    size_t passwordlen;
    ...
    int ret = iot_crypto_pbkdf2_sha256((const uint8_t*)password, passwordlen,
...
/* crypto 模块方法表 */
static const luaL_Reg iot_crypto_methods[] = {
    { "hash",         luaopen_iot_crypto_hash },
    { "hmac",         luaopen_iot_crypto_hmac },
    { "encrypt",      luaopen_iot_crypto_encrypt },
    { "decrypt",      luaopen_iot_crypto_decrypt },
    { "rand",         luaopen_iot_crypto_rand },
    { "hex2bin",      luaopen_iot_crypto_hex2bin },
    { "bin2hex",      luaopen_iot_crypto_bin2hex },
    { "base64_encode",luaopen_iot_crypto_base64_encode },
    { "base64_decode",luaopen_iot_crypto_base64_decode },
    { "x509_parse_pem",luaopen_iot_crypto_x509_parse_pem },
    { "pbkdf2_sha256",luaopen_iot_crypto_pbkdf2_sha256 },
    { NULL,           NULL }
};

LUAMOD_API int luaopen_iot_crypto_register(lua_State* L) {
    luaL_newlib(L, iot_crypto_methods);
    ...
    /* 注册 X509 证书类型元表 */
    luaL_newmetatable(L, IOT_X509_CERT_METATABLE);
    lua_pushcfunction(L, luaopen_iot_crypto_x509_cert_gc);
    lua_setfield(L, -2, "__gc");
    lua_pushcfunction(L, luaopen_iot_crypto_x509_cert_tostring);
    lua_setfield(L, -2, "__tostring");
    ...
    lua_setfield(L, -2, "__index");
    /* 注册 X509 证书方法 */
    luaL_setfuncs(L, iot_x509_cert_methods, 0);
    lua_pop(L, 1);  /* 弹出元表 */
