...
*/
static int iot_lvgl_img_create(lua_State* L) {
    return iot_lvgl_obj_create_instance(L, iot_lvgl_img_create_internal, img_metatable_ref);
}
...
*/
static int iot_lvgl_img_set_pivot(lua_State* L) {
    lv_obj_t* img = iot_lvgl_get_obj_ptr(L, 1);
...
void iot_lvgl_register_img(lua_State* L) {
    ...
    /* 注册OO风格方法 */
    REG_METHOD(L, "set_src", iot_lvgl_img_set_src);
    REG_METHOD(L, "set_offset_x", iot_lvgl_img_set_offset_x);
    ...
    REG_METHOD(L, "create", iot_lvgl_img_create);
}
