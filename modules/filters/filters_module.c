...
typedef struct {
    ...
    union {
        ...
        filter_iir_t iir;
        filter_lowpass_t lowpass;
        filter_complementary_t complementary;
    } impl;
} filter_lua_obj_t;

#define FILTER_LUA_MT "filter"
...
{
    (void)unused;
    return filter_ema_update((filter_ema_t*)impl, input, output);
}
static int ema_get(void* impl, float* output)
{
    return filter_ema_get((filter_ema_t*)impl, output);
}
static void ema_reset(void* impl, float initial)
{
    filter_ema_t* f = (filter_ema_t*)impl;
    filter_ema_reset(f);
    if (initial != 0.0f) {
        f->value = initial;
        f->initialized = true;
...
{
    (void)unused;
    return filter_ma_update((filter_ma_t*)impl, input, output);
}
static int ma_get(void* impl, float* output)
{
    return filter_ma_get((filter_ma_t*)impl, output);
}
static void ma_reset(void* impl, float initial)
{
    (void)initial;
    filter_ma_reset((filter_ma_t*)impl);
}
static void ma_deinit(void* impl)
{
    filter_ma_deinit((filter_ma_t*)impl);
}
...
{
    (void)unused;
    return filter_median_update((filter_median_t*)impl, input, output);
}
static int median_get(void* impl, float* output)
{
    return filter_median_get((filter_median_t*)impl, output);
}
static void median_reset(void* impl, float initial)
{
    (void)initial;
    filter_median_reset((filter_median_t*)impl);
}
static void median_deinit(void* impl)
{
    filter_median_deinit((filter_median_t*)impl);
}
...
{
    (void)unused;
    return filter_kalman_update((filter_kalman_t*)impl, input, output);
}
static int kalman_get(void* impl, float* output)
{
    return filter_kalman_get((filter_kalman_t*)impl, output);
}
static void kalman_reset(void* impl, float initial)
{
    filter_kalman_t* f = (filter_kalman_t*)impl;
    filter_kalman_reset(f);
    if (initial != 0.0f) {
        f->x = initial;
        f->initialized = true;
    }
...
{
    (void)unused;
    return filter_iir_update((filter_iir_t*)impl, input, output);
}
static int iir_get(void* impl, float* output)
{
    return filter_iir_get((filter_iir_t*)impl, output);
}
static void iir_reset(void* impl, float initial)
{
    (void)initial;
    filter_iir_reset((filter_iir_t*)impl);
}
...
{
    (void)unused;
    return filter_lowpass_update((filter_lowpass_t*)impl, input, output);
}
static int lowpass_get(void* impl, float* output)
{
    return filter_lowpass_get((filter_lowpass_t*)impl, output);
}
static void lowpass_reset(void* impl, float initial)
{
    filter_lowpass_t* f = (filter_lowpass_t*)impl;
    filter_lowpass_reset(f);
    if (initial != 0.0f) {
        f->value = initial;
        f->initialized = true;
    }
...
{
    return filter_complementary_update((filter_complementary_t*)impl, input, input2, output);
}
static int complementary_get(void* impl, float* output)
{
    return filter_complementary_get((filter_complementary_t*)impl, output);
}
static void complementary_reset(void* impl, float initial)
{
    filter_complementary_t* f = (filter_complementary_t*)impl;
    filter_complementary_reset(f);
    if (initial != 0.0f) {
        f->value = initial;
        f->initialized = true;
    }
 ...
 */
static int filter_lua_update(lua_State* L)
{
    filter_lua_obj_t* obj = (filter_lua_obj_t*)luaL_checkudata(L, 1, FILTER_LUA_MT);
    float input = (float)luaL_checknumber(L, 2);
    float input2 = 0.0f;
    
    /* complementary 类型接收两个参数 */
    if (obj->type == FILTER_LUA_COMPLEMENTARY) {
...
static int filter_lua_get(lua_State* L)
{
    filter_lua_obj_t* obj = (filter_lua_obj_t*)luaL_checkudata(L, 1, FILTER_LUA_MT);
    float output;
    if (obj->get(&obj->impl, &output) != FILTER_OK) {
...
static const luaL_Reg filter_methods[] = {
    { "update", filter_lua_update },
    { "get",    filter_lua_get },
...
static int create_ema(lua_State* L, int opts_idx)
{
    ...
    if (alpha < 0.0f) {
        ...
        if (cutoff > 0.0f && sr > 0.0f) {
            alpha = filter_calc_alpha_from_freq(cutoff, sr);
        }
    }
    if (alpha < 0.0f) {
        ...
        float sr  = get_float_field(L, opts_idx, "sr", 0.0f);
        if (tau > 0.0f && sr > 0.0f) {
            alpha = filter_calc_alpha_from_tau(tau, sr);
        }
    }
    ...
    filter_lua_obj_t* obj = (filter_lua_obj_t*)lua_newuserdata(L, sizeof(filter_lua_obj_t));
    memset(obj, 0, sizeof(*obj));
    obj->type = FILTER_LUA_EMA;
    obj->update = ema_update;
    obj->get = ema_get;
    obj->reset = ema_reset;
    obj->deinit = ema_deinit;

    filter_ema_init(&obj->impl.ema, alpha);
    ...
    luaL_getmetatable(L, FILTER_LUA_MT);
...
static int create_ma(lua_State* L, int opts_idx)
{
    ...
    filter_lua_obj_t* obj = (filter_lua_obj_t*)lua_newuserdata(L, sizeof(filter_lua_obj_t));
    memset(obj, 0, sizeof(*obj));
    obj->type = FILTER_LUA_MA;
    obj->update = ma_update;
    obj->get = ma_get;
    obj->reset = ma_reset;
    obj->deinit = ma_deinit;

    if (filter_ma_init(&obj->impl.ma, window) != FILTER_OK) {
...
static int create_median(lua_State* L, int opts_idx)
{
    ...
    filter_lua_obj_t* obj = (filter_lua_obj_t*)lua_newuserdata(L, sizeof(filter_lua_obj_t));
    memset(obj, 0, sizeof(*obj));
    obj->type = FILTER_LUA_MEDIAN;
    obj->update = median_update;
    obj->get = median_get;
    obj->reset = median_reset;
    obj->deinit = median_deinit;

    if (filter_median_init(&obj->impl.median, window) != FILTER_OK) {
...
static int create_iir(lua_State* L, int opts_idx)
{
    ...
    filter_lua_obj_t* obj = (filter_lua_obj_t*)lua_newuserdata(L, sizeof(filter_lua_obj_t));
    memset(obj, 0, sizeof(*obj));
    obj->type = FILTER_LUA_IIR;
    obj->update = iir_update;
    obj->get = iir_get;
    obj->reset = iir_reset;
    obj->deinit = iir_deinit;

    if (filter_iir_init(&obj->impl.iir, b, a, order) != FILTER_OK) {
 ...
 */
static int luaopen_filter_create(lua_State* L)
{
...
static int luaopen_filter_alpha_from_freq(lua_State* L)
{
    float cutoff      = (float)luaL_checknumber(L, 1);
    float sample_rate = (float)luaL_checknumber(L, 2);
    lua_pushnumber(L, filter_calc_alpha_from_freq(cutoff, sample_rate));
    return 1;
}

static int luaopen_filter_alpha_from_tau(lua_State* L)
{
    float tau         = (float)luaL_checknumber(L, 1);
    float sample_rate = (float)luaL_checknumber(L, 2);
    lua_pushnumber(L, filter_calc_alpha_from_tau(tau, sample_rate));
    return 1;
}
...
static const luaL_Reg filter_module_funcs[] = {
    { "create",            luaopen_filter_create },
    { "alpha_from_freq",   luaopen_filter_alpha_from_freq },
    { "alpha_from_tau",    luaopen_filter_alpha_from_tau },
    { NULL,                NULL }
};
...
{
    luaL_newlib(L, filter_module_funcs);

    /* 注册统一的滤波器元表 */
    luaL_newmetatable(L, FILTER_LUA_MT);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_setfuncs(L, filter_methods, 0);
    lua_pop(L, 1);
