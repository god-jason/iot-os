 ...
 * Private Data
 ****************************************************************************/
static STRUCT_BMP180_TYPEDEF bmp180 = {0}; /* 定义结构体用来保存bmp180传感器参数 */
...
static void cm_i2c_iomux_set(cm_i2c_dev_e dev)
{
    if(dev == CM_I2C_DEV_0)
    {
        cm_iomux_set_pin_func(CM_IOMUX_PIN_46, CM_IOMUX_FUNC_FUNCTION1); 
        cm_iomux_set_pin_func(CM_IOMUX_PIN_45, CM_IOMUX_FUNC_FUNCTION1);
    }
    else if(dev == CM_I2C_DEV_1)
    {
        cm_iomux_set_pin_func(CM_IOMUX_PIN_87, CM_IOMUX_FUNC_FUNCTION2); 
        cm_iomux_set_pin_func(CM_IOMUX_PIN_89, CM_IOMUX_FUNC_FUNCTION2); 
    }
    else if(dev == CM_I2C_DEV_2)
    {
        cm_iomux_set_pin_func(CM_IOMUX_PIN_88, CM_IOMUX_FUNC_FUNCTION3); 
        cm_iomux_set_pin_func(CM_IOMUX_PIN_90, CM_IOMUX_FUNC_FUNCTION3); 
    }
    else
    {
...
static int32_t is24c256_write_byte(uint16_t addr, int8_t data)
{
    ...
    ret = cm_i2c_write(i2c_dev , EPROM_DEV_ADDR, tmp, 3);
    if(ret < 0)
    {
...
static int32_t is24c256_read_byte(uint16_t addr, int8_t* data)
{
    ...
    ret = cm_i2c_write(i2c_dev, EPROM_DEV_ADDR, tmp, 2);
    if(ret < 0)
    {
    ...
    ret = cm_i2c_read(i2c_dev, EPROM_DEV_ADDR, data, 1);
    if(ret < 0)
    {
 ...
 static int16_t bmp180_multiRead(uint8_t reg_addr)
 {
     ...
     ret = cm_i2c_write_then_read(i2c_dev, BMP180_SLAVE_ADDR, tmp, 1,data,2);
     if(ret < 0)
     {
 ...
 static int16_t bmp180_read_temperature_reg(void)
 {
     ...
     ret = cm_i2c_write(i2c_dev, 0x77, tmp, 2);
     if(ret < 0)
     {
         CM_DEMO_I2C_LOG("L:%d,i2c read addr err(w):%08x\r\n", __LINE__, ret);
         return ret;
 ...
 static uint8_t bmp180_i2c_init(void)
{
    ...
    cm_i2c_iomux_set(i2c_dev);

    ret = cm_i2c_open(i2c_dev, &config);
    if(ret != 0)
    {
...
static void bmp180_measure(void)
{
    ...
    /* ------------------------ 温度值计算 ------------------------ */
    x1 = ((reg_temp - (long)bmp180.adjust.ac6)*(long)bmp180.adjust.ac5) >> 15;
    x2 = ((long) bmp180.adjust.mc << 11) / (x1 + bmp180.adjust.md);
    b5 = x1 + x2;
    bmp180.temperature = (((b5 + 8) >> 4) / 10.f) + ((float)(((b5 + 8) >> 4) % 10) / 10.f); /* 获取温度值 */
    /* ------------------------ 气压值计算 ------------------------ */
    b6 = b5 - 4000;
    /* 计算 b3 */
    ...
    /* ------------------------ 海拔高度计算 ------------------------ */
    bmp180.altitude = (BMP180_ZERO_ALTITUDE - p) / 133.0 * 12.0;
...
static int8_t cm_i2c_m117b(void)
{
    ...
    send_buf[0] = 0x30;
    send_buf[1] = 0xa2;
    write_len = cm_i2c_write(i2c_dev, M177B_DEV_ADDR, send_buf, 2);
    if(write_len != 2)
    {
    ...
    read_len = cm_i2c_read(i2c_dev,M177B_DEV_ADDR,read_data,3);
    if(read_len != 3)
    {
...
void cm_test_i2c(EmbeddedCli *cli, char *args, void *context)
{
    ...
    if (strncmp(cmd, STR_ITEM("eeprom")) == 0)
    {
        ...
        i2c_dev = atoi((char *)param[0]);
        cm_test_i2c_eeprom();
    }
    else if(strncmp(cmd, STR_ITEM("bmp180")) == 0)
    {
        if (parse_params(args, param, param_num) != 0)
        {
            return;
    ...
    else if(strncmp(cmd, STR_ITEM("m177b")) == 0)
    {
        if (parse_params(args, param, param_num) != 0)
        {
            return;
