...
static int cm_i2c_init(void)
{
    ...
    int ret = cm_i2c_open(ES8311_I2C_ID, &config);
    if(ret != 0)
    {
...
int cm_test_i2c(uint8_t reg)
{
    ...
    ret = cm_i2c_write(ES8311_I2C_ID , ES8311_DEV_ADDR, &es8311_addr, 1);
    if(ret != 0)
    {
    ...
    ret = cm_i2c_read(ES8311_I2C_ID , ES8311_DEV_ADDR, &r_data, 1);
    if(ret != 0)
    {
        CM_DEMO_I2C_LOG("cm_i2c_read err %d\n", (uint32_t)ret);
        cm_i2c_close(ES8311_I2C_ID);
        return -2;
    }
...
static void read_es8311_id(void)
{
    ...
    while(cnt --)
    {
        ...
        ret = cm_test_i2c(ES8311_CHD2_REGFE);
        if(ret < 0)
        {
            CM_DEMO_I2C_LOG("cm_test_i2c err ret %d!!\n",ret);
        }
...
static int tc_read_es8311_id_abnormal(void)
{
    ...
    ret = cm_i2c_write(ES8311_I2C_ID , slave_addr, &register_addr, 1);
    if (ret != 0)
    {
    ...
    ret = cm_i2c_read(ES8311_I2C_ID , slave_addr, &r_data, 1);
    if (ret != 0)
    {
        CM_DEMO_I2C_LOG("cm_i2c_read err3[%d]", ret);
    }
    ...
    if (ret != 0)
    {
        CM_DEMO_I2C_LOG("cm_i2c_read err5[%d]", ret);
    }
    CM_DEMO_I2C_LOG("i2c read es8311 r_data[%02x]", r_data);
...
static int tc_read_es8311_id_abnormal_1(void)
{
    ...
    // test with right slave addr
    ret = cm_i2c_open(ES8311_I2C_ID, &config);
    if(ret != 0)
    {
    ...
    if(ret != 0)
    {
        CM_DEMO_I2C_LOG("cm_i2c_read err7[%d]", ret);
    }
    CM_DEMO_I2C_LOG("i2c read es8311 r_data[%02X]", r_data);
...
static int tc_read_es8311_id_abnormal_2(void)
{
    ...
    if (ret != 0)
    {
        CM_DEMO_I2C_LOG("cm_i2c_read slave_addr[%X] err[%d]", slave_addr, ret);
    }
    else
    {
                                         ...
                                         cm_i2c_mode_e mode, int clock)
{
    ...
    if (mode <= CM_I2C_MODE_SLAVER)
    {
        config.mode = mode;
    }
    if ((clock == CM_I2C_CLK_100KHZ) || (clock == CM_I2C_CLK_400KHZ) ||
        (clock == CM_I2C_CLK_HS_KHZ) || (clock == CM_I2C_CLK_HS_FAST_KHZ))
    {
        config.clk = clock;
    }
    CM_DEMO_I2C_LOG("dev[%d]addr_type[%d]mode[%d]clock[%d]\n", dev, addr_type, mode, clock);

    int ret = cm_i2c_open(dev, &config);
    if(ret != 0)
    {
        CM_DEMO_I2C_LOG("cm_i2c_open err ret[%d]\n", ret);
    }
                                  ...
                                  uint8_t * p_data, uint32_t data_len)
{
    ...
    {
        //读取数据
        ret = cm_i2c_read(dev, slave_addr, p_data, data_len);
        if(ret != 0)
        {
            CM_DEMO_I2C_LOG("cm_i2c_read[%d]serr[%d]\n", dev, ret);
            cm_i2c_close(dev);
            return -2;
        }
        CM_DEMO_I2C_LOG("i2c read es8311 ret %d, r_data[%02x]\n", ret, *p_data);
    ...
    {
        //写入测试数据
        ret = cm_i2c_write(dev , slave_addr, p_data, data_len);
        if(ret != 0)
        {
            CM_DEMO_I2C_LOG("cm_i2c_write[%d]err[%d]\n", dev, ret);
            cm_i2c_close(dev);
            return -3;
        }
        CM_DEMO_I2C_LOG("i2c write es8311 ok\n");
