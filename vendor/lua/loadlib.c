...
/*
** 默认的Lua模块搜索路径前缀
** require默认从/app/目录加载模块
*/
#define LUA_APP_PREFIX "/app/"
...
**
** @note 嵌入式系统不支持动态库加载，此功能已禁用
*/
#if defined(LUA_USE_EMBEDDED_DISABLE_CLOAD)
static int lookforfunc (lua_State *L, const char *path, const char *sym) {
  (void)path;
  (void)sym;
  lua_pushstring(L, "C library loading is not supported on this platform");
  return ERRLIB;
}
#else
static int lookforfunc (lua_State *L, const char *path, const char *sym) {
...
/*
** @brief 加载动态库函数（已禁用，嵌入式系统不支持动态库加载）
*/
#if defined(LUA_USE_EMBEDDED_DISABLE_CLOAD)
static int ll_loadlib (lua_State *L) {
  (void)L;
  return luaL_error(L, "C library loading is not supported on this platform");
}
#else
static int ll_loadlib (lua_State *L) {
...
/*
** 原始的searcher_Lua函数，使用package.path搜索模块
*/
...
/*
** @brief 从/app/目录搜索Lua模块，优先加载.luac后缀文件
** @param L Lua状态机
** @return 0 成功加载，1 未找到模块
**
** 搜索策略：
...
static int searcher_app (lua_State *L) {
  ...
  /* 构建.luac路径 */
  strcpy(luac_path, LUA_APP_PREFIX);
  strcat(luac_path, name);
  strcat(luac_path, ".luac");

  /* 构建.lua路径 */
  strcpy(lua_path, LUA_APP_PREFIX);
  strcat(lua_path, name);
  strcat(lua_path, ".lua");

  /* 优先查找.luac文件 */
  if (file_readable(luac_path)) {
    filename = luac_path;
  } else if (file_readable(lua_path)) {
    /* .luac不存在，查找.lua文件 */
    filename = lua_path;
  } else {
    /* 都找不到，返回错误 */
...
/*
** @brief C库搜索函数（已禁用，嵌入式系统不支持动态库加载）
*/
#if defined(LUA_USE_EMBEDDED_DISABLE_CLOAD)
...
/*
** @brief C库根搜索函数（已禁用，嵌入式系统不支持动态库加载）
*/
#if defined(LUA_USE_EMBEDDED_DISABLE_CLOAD)
...
static void createsearcherstable (lua_State *L) {
  static const lua_CFunction searchers[] =
    {searcher_preload, searcher_app, searcher_Lua, searcher_C, searcher_Croot, NULL};
  int i;
