...
/*
** {======================================================
** EXTENSION: Additional string operations
** =======================================================
*/
...
内部trim实现
*/
static int str_trim_impl (lua_State *L, int ltrim, int rtrim) {
  size_t str_len = 0;
  const char *str = luaL_checklstring(L, 1, &str_len);
  if (str_len == 0) {
    lua_pushvalue(L, 1);
...
static int str_trim (lua_State *L) {
  ...
  return str_trim_impl(L, ltrim, rtrim);
}
...
static int str_toHex (lua_State *L) {
  ...
  luaL_Buffer b;
  size_t buff_size = (2 + sep_len) * len;
  luaL_buffinitsize(L, &b, buff_size);

  for (size_t i = 0; i < len; i++) {
    unsigned char c = (unsigned char)str[i];
    luaL_addchar(&b, "0123456789ABCDEF"[c >> 4]);
    luaL_addchar(&b, "0123456789ABCDEF"[c & 0xF]);
    /* 添加分隔符（如果不是最后一个字节） */
