...
int driver_i2c_init(driver_i2c_t* i2c, int bus, int speed) {
    if (!i2c) {
        LOG_INFO("[DRV] I2C init failed: invalid parameter");
    ...
    if (i2c->fd >= 0) {
        LOG_INFO("[DRV] I2C bus %d initialized", bus);
        return DRIVER_OK;
    }
    LOG_INFO("[DRV] I2C bus %d init failed", bus);
...
int driver_spi_init(driver_spi_t* spi, int bus, int cs, int speed, int mode) {
    if (!spi) {
        LOG_INFO("[DRV] SPI init failed: invalid parameter");
        return DRIVER_ERR_INVALID;
    }
    ...
    if (spi->fd >= 0) {
        LOG_INFO("[DRV] SPI bus %d cs %d initialized", bus, cs);
        return DRIVER_OK;
    }
    LOG_INFO("[DRV] SPI bus %d cs %d init failed", bus, cs);
...
int driver_register(driver_t* drv) {
    if (!drv || !drv->name) {
        LOG_INFO("[DRV] Register failed: invalid parameter");
        return DRIVER_ERR_INVALID;
    }
    ...
    LOG_INFO("[DRV] Driver '%s' registered", drv->name);
    return DRIVER_OK;
}
